...
<body>
    <main class="container">
        ...
        <div class="row mt-5 ms-3">
            <div class="col-12">
                <p>Stil Deposunda'ndan yaptğınız siparişiniz işleme alınmıştır!.</p>
            </div>
            <div class="col-12">
        ...
        <div id="finish" class="row mt-5 w-100"
            ...
            @if ($products)
                @foreach ($products as $brandProducts)
                    @foreach ($brandProducts as $productData)
                        @php
                            $product = $productData['product'];
                            $price = (float) $product['product_detail']['price'];
                            $totalPrice += $price * $productData['count'];
                        @endphp
                        ...
                        <div class="col-2 mt-2">
                            <span style="opacity: 0.8; font-size:15px;">
                                Adet: {{ $productData['count'] }}
                            </span>
                        </div>
                        <div class="col-2 mt-2">
                            <span style="opacity: 0.8; font-size:15px;color:#f37a1b">
                                {{ $product['product_detail']['price'] * $productData['count'] }} TL
                            </span>
                        </div>
                    @endforeach
                @endforeach
        ...
        @php
            $adress = json_decode($adress, true);
        @endphp
